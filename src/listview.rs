...
pub trait Listable {
    ...
    fn on_key(&mut self, _key: Key) -> WResult<()> {
        Ok(())
    }
}
...
impl ListView<Files> {
    ...
    pub fn select_file(&mut self, file: &File) {
        ...
        match posfile {
            ...
            None => {
                ...
                WError::wrong_directory::<()>(dir.clone(), file.clone()).log();
                let file = self
                    .content
                    .iter_files()
                    .nth(0)
    ...
    fn search_next(&mut self) -> WResult<()> {
        ...
        let prev_search = self.searching.clone().ok_or(WError::NoneError)?;
    ...
    fn filter(&mut self) -> WResult<()> {
        ...
        loop {
            ...
            match filter {
                Err(WError::MiniBufferEvent(event)) => match event {
                    Done(filter) => {
                        self.core
