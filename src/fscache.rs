...
impl FsEventDispatcher {
    ...
    fn add_target(&self, dir: &File, target: &Arc<RwLock<Vec<FsEvent>>>) -> WResult<()> {
...
impl FsCache {
    ...
    pub fn apply_settingss(cache: &FsCache, files: &mut Files) -> WResult<()> {
        let dir = &files.directory;
        let tab_settings = cache.tab_settings.read()?.get(&dir).cloned();
        if tab_settings.is_none() {
            return Ok(());
        }
        let tab_settings = tab_settings.ok_or(WError::NoneError)?;
...
impl TryFrom<DebouncedEvent> for FsEvent {
    ...
    fn try_from(event: DebouncedEvent) -> WResult<Self> {
        let event = match event {
            ...
            DebouncedEvent::Rescan => Err(WError::INotifyError("Need to rescan".to_string()))?,
            // Ignore NoticeRemove/NoticeWrite
            _ => None.ok_or(WError::NoneError)?,
        };
...
fn watch_fs(
    ...
    std::thread::spawn(move || -> WResult<()> {
        let transform_event = move |event: DebouncedEvent| -> WResult<(File, FsEvent)> {
        ...
        let collect_events = move || -> WResult<HashMap<File, Vec<FsEvent>>> {
            let event = loop {
                use DebouncedEvent::*;
        ...
        let dispatch_events = move |events| -> WResult<()> {
