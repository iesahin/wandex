...
use crate::fail::{ErrorLog, KeyBindError, WError, WResult};
...
impl FoldableWidgetExt for ListView<Vec<QuickActions>> {
    ...
    fn render_header(&self) -> WResult<String> {
        let mime = &self.content.get(0).ok_or(WError::NoneError)?.mime;
...
impl ListView<Vec<QuickActions>> {
    ...
    fn run_action(&mut self, num: Option<usize>) -> WResult<()> {
        ...
        let current_fold = self.current_fold().ok_or(WError::NoneError)?;
        let fold_start_pos = self.fold_start_pos(current_fold);
        let selection = self.get_selection();
...
impl QuickAction {
    ...
    fn run(
        ...
        let cwd = files.get(0).ok_or(WError::NoneError)?.parent_as_file()?;
